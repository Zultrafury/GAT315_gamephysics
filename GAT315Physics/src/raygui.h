...
RAYGUIAPI int GuiSlider(Rectangle bounds, const char* textLeft, const char* textRight, float* value, float minValue,
                        float maxValue); // Slider control
RAYGUIAPI int GuiSliderBar(Rectangle bounds, const char* textLeft, const char* textRight, float* value, float minValue,
                           float maxValue); // Slider Bar control
RAYGUIAPI int GuiProgressBar(Rectangle bounds, const char* textLeft, const char* textRight, float* value,
                             float minValue, float maxValue); // Progress Bar control
...
void GuiSetStyle(int control, int property, int value)
{
    ...
    {
        for (int i = 1; i < RAYGUI_MAX_CONTROLS; i++) guiStyle[i * (RAYGUI_MAX_PROPS_BASE + RAYGUI_MAX_PROPS_EXTENDED) +
            property] = value;
    }
}
...
#define RAYGUI_WINDOWBOX_CLOSEBUTTON_HEIGHT      18
    ...
    Rectangle windowPanel = {
        bounds.x, bounds.y + (float)statusBarHeight - 1, bounds.width, bounds.height - (float)statusBarHeight + 1
    };
    Rectangle closeButtonRec = {
        statusBar.x + statusBar.width - GuiGetStyle(STATUSBAR, BORDER_WIDTH) - RAYGUI_WINDOWBOX_CLOSEBUTTON_HEIGHT -
        vPadding,
...
#define RAYGUI_GROUPBOX_LINE_THICK     1
    ...
    // Draw control
    //--------------------------------------------------------------------
    GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){bounds.x, bounds.y, RAYGUI_GROUPBOX_LINE_THICK, bounds.height}, 0,
                     BLANK, GetColor(GuiGetStyle(
                         DEFAULT, (state == STATE_DISABLED) ? (int)BORDER_COLOR_DISABLED : (int)LINE_COLOR)));
    GuiDrawRectangle(
        RAYGUI_CLITERAL(Rectangle){bounds.x, bounds.y + bounds.height - 1, bounds.width, RAYGUI_GROUPBOX_LINE_THICK}, 0,
        BLANK, GetColor(
            GuiGetStyle(DEFAULT, (state == STATE_DISABLED) ? (int)BORDER_COLOR_DISABLED : (int)LINE_COLOR)));
    GuiDrawRectangle(
        RAYGUI_CLITERAL(Rectangle){bounds.x + bounds.width - 1, bounds.y, RAYGUI_GROUPBOX_LINE_THICK, bounds.height}, 0,
        BLANK, GetColor(
            GuiGetStyle(DEFAULT, (state == STATE_DISABLED) ? (int)BORDER_COLOR_DISABLED : (int)LINE_COLOR)));

    GuiLine(RAYGUI_CLITERAL(Rectangle){
...
#define RAYGUI_LINE_TEXT_PADDING  4
    ...
    // Draw control
    //--------------------------------------------------------------------
    if (text == NULL) GuiDrawRectangle(
        RAYGUI_CLITERAL(Rectangle){bounds.x, bounds.y + bounds.height / 2, bounds.width, 1}, 0, BLANK, color);
    else
    {
        ...
        GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){
                             bounds.x, bounds.y + bounds.height / 2, RAYGUI_LINE_MARGIN_TEXT - RAYGUI_LINE_TEXT_PADDING,
                             1
                         }, 0, BLANK, color);
        GuiDrawText(text, textBounds, TEXT_ALIGN_LEFT, color);
        GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){
                             bounds.x + 12 + textBounds.width + 4, bounds.y + bounds.height / 2,
                             bounds.width - textBounds.width - RAYGUI_LINE_MARGIN_TEXT - RAYGUI_LINE_TEXT_PADDING, 1
                         }, 0, BLANK, color);
    }
    //--------------------------------------------------------------------
...
#define RAYGUI_PANEL_BORDER_WIDTH   1
    ...
    if ((text != NULL) && (bounds.height < RAYGUI_WINDOWBOX_STATUSBAR_HEIGHT * 2.0f)) bounds.height =
        RAYGUI_WINDOWBOX_STATUSBAR_HEIGHT * 2.0f;
...
#define RAYGUI_TABBAR_ITEM_WIDTH    160
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (tabBounds.x < GetScreenWidth())
        {
            ...
            if (CheckCollisionPointRec(GetMousePosition(), tabBounds) && IsMouseButtonPressed(MOUSE_MIDDLE_BUTTON))
                result = i;
...
#define RAYGUI_MIN_MOUSE_WHEEL_SPEED   20
    ...
    // Text will be drawn as a header bar (if provided)
    Rectangle statusBar = {bounds.x, bounds.y, bounds.width, (float)RAYGUI_WINDOWBOX_STATUSBAR_HEIGHT};
    if (bounds.height < RAYGUI_WINDOWBOX_STATUSBAR_HEIGHT * 2.0f) bounds.height = RAYGUI_WINDOWBOX_STATUSBAR_HEIGHT *
        2.0f;
    ...
    bool hasHorizontalScrollBar = (content.width > bounds.width - 2 * GuiGetStyle(DEFAULT, BORDER_WIDTH))
                                      ? true
                                      : false;
    ...
    if (!hasHorizontalScrollBar) hasHorizontalScrollBar = (hasVerticalScrollBar && (content.width > (bounds.width - 2 *
                                                              GuiGetStyle(DEFAULT, BORDER_WIDTH) - GuiGetStyle(
                                                                  LISTVIEW, SCROLLBAR_WIDTH))))
                                                              ? true
                                                              : false;
    if (!hasVerticalScrollBar) hasVerticalScrollBar = (hasHorizontalScrollBar && (content.height > (bounds.height - 2 *
                                                          GuiGetStyle(DEFAULT, BORDER_WIDTH) - GuiGetStyle(
                                                              LISTVIEW, SCROLLBAR_WIDTH))))
                                                          ? true
                                                          : false;
    ...
    Rectangle verticalScrollBar = {
        (float)((GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE)
                    ? (float)bounds.x + GuiGetStyle(DEFAULT, BORDER_WIDTH)
                    : (float)bounds.x + bounds.width - verticalScrollBarWidth - GuiGetStyle(DEFAULT, BORDER_WIDTH)),
    ...
    *view = (GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE)
    ...
    float horizontalMin = hasHorizontalScrollBar
                              ? ((GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE)
                                     ? (float)-verticalScrollBarWidth
                                     : 0) - (float)GuiGetStyle(DEFAULT, BORDER_WIDTH)
                              : (((float)GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE)
                                     ? (float)-verticalScrollBarWidth
                                     : 0) - (float)GuiGetStyle(DEFAULT, BORDER_WIDTH);
    ...
    float verticalMax = hasVerticalScrollBar
                            ? content.height - bounds.height + (float)horizontalScrollBarWidth + (float)
                            GuiGetStyle(DEFAULT, BORDER_WIDTH)
                            : (float)-GuiGetStyle(DEFAULT, BORDER_WIDTH);
    ...
    if ((state != STATE_DISABLED) && !guiLocked)
    {
        ...
        if (CheckCollisionPointRec(mousePoint, bounds))
        {
            ...
            if (hasHorizontalScrollBar && (IsKeyDown(KEY_LEFT_CONTROL) || IsKeyDown(KEY_LEFT_SHIFT))) scrollPos.x +=
                wheelMove * mouseWheelSpeed.x;
    ...
    if (hasHorizontalScrollBar)
    {
        ...
        GuiSetStyle(SCROLLBAR, SCROLL_SLIDER_SIZE,
                    (int)(((bounds.width - 2 * GuiGetStyle(DEFAULT, BORDER_WIDTH) - verticalScrollBarWidth) / (int)
                        content.width) * ((int)bounds.width - 2 * GuiGetStyle(DEFAULT, BORDER_WIDTH) -
                        verticalScrollBarWidth)));
        scrollPos.x = (float)-GuiScrollBar(horizontalScrollBar, (int)-scrollPos.x, (int)horizontalMin,
                                           (int)horizontalMax);
    }
    ...
    if (hasVerticalScrollBar)
    {
        ...
        GuiSetStyle(SCROLLBAR, SCROLL_SLIDER_SIZE,
                    (int)(((bounds.height - 2 * GuiGetStyle(DEFAULT, BORDER_WIDTH) - horizontalScrollBarWidth) / (int)
                        content.height) * ((int)bounds.height - 2 * GuiGetStyle(DEFAULT, BORDER_WIDTH) -
    ...
    if (hasHorizontalScrollBar && hasVerticalScrollBar)
    {
        Rectangle corner = {
            ...
            verticalScrollBar.y + verticalScrollBar.height + 2, (float)horizontalScrollBarWidth - 4,
    ...
    GuiDrawRectangle(bounds, GuiGetStyle(LISTVIEW, BORDER_WIDTH), GetColor(GuiGetStyle(LISTVIEW, BORDER + (state * 3))),
                     BLANK);
...
int GuiLabel(Rectangle bounds, const char* text)
{
    ...
    // Draw control
    //--------------------------------------------------------------------
    GuiDrawText(text, GetTextBounds(LABEL, bounds), GuiGetStyle(LABEL, TEXT_ALIGNMENT),
...
int GuiButton(Rectangle bounds, const char* text)
{
    ...
    // Draw control
    //--------------------------------------------------------------------
    GuiDrawRectangle(bounds, GuiGetStyle(BUTTON, BORDER_WIDTH), GetColor(GuiGetStyle(BUTTON, BORDER + (state * 3))),
...
int GuiLabelButton(Rectangle bounds, const char* text)
{
    ...
    if ((bounds.width - 2 * GuiGetStyle(LABEL, BORDER_WIDTH) - 2 * GuiGetStyle(LABEL, TEXT_PADDING)) < textWidth) bounds
        .width = textWidth + 2 * GuiGetStyle(LABEL, BORDER_WIDTH) + 2 * GuiGetStyle(LABEL, TEXT_PADDING) + 2;
...
int GuiToggle(Rectangle bounds, const char* text, bool* active)
{
    ...
    if (state == STATE_NORMAL)
    {
        ...
        GuiDrawText(text, GetTextBounds(TOGGLE, bounds), GuiGetStyle(TOGGLE, TEXT_ALIGNMENT),
                    GetColor(GuiGetStyle(TOGGLE, ((*active) ? TEXT_COLOR_PRESSED : (TEXT + state * 3)))));
    }
    else
    {
        GuiDrawRectangle(bounds, GuiGetStyle(TOGGLE, BORDER_WIDTH), GetColor(GuiGetStyle(TOGGLE, BORDER + state * 3)),
...
int GuiToggleSlider(Rectangle bounds, const char* text, int* active)
{
    ...
    Rectangle slider = {
        ...
        (bounds.width - 2 * GuiGetStyle(SLIDER, BORDER_WIDTH) - (itemCount + 1) * GuiGetStyle(SLIDER, SLIDER_PADDING)) /
        itemCount,
    ...
    slider.x = bounds.x + GuiGetStyle(SLIDER, BORDER_WIDTH) + (*active + 1) * GuiGetStyle(SLIDER, SLIDER_PADDING) + (*
        active) * slider.width;
    //--------------------------------------------------------------------
    ...
    else if (state == STATE_FOCUSED) GuiDrawRectangle(slider, 0, BLANK,
                                                      GetColor(GuiGetStyle(SLIDER, BASE_COLOR_FOCUSED)));
    else if (state == STATE_PRESSED) GuiDrawRectangle(slider, 0, BLANK,
    ...
    if (text != NULL)
    {
        ...
        GuiDrawText(items[*active], textBounds, GuiGetStyle(TOGGLE, TEXT_ALIGNMENT),
...
int GuiCheckBox(Rectangle bounds, const char* text, bool* checked)
{
    ...
    if (text != NULL)
    {
        ...
        textBounds.y = bounds.y + bounds.height / 2 - GuiGetStyle(DEFAULT, TEXT_SIZE) / 2;
        if (GuiGetStyle(CHECKBOX, TEXT_ALIGNMENT) == TEXT_ALIGN_LEFT) textBounds.x = bounds.x - textBounds.width -
    ...
    // Draw control
    //--------------------------------------------------------------------
    GuiDrawRectangle(bounds, GuiGetStyle(CHECKBOX, BORDER_WIDTH), GetColor(GuiGetStyle(CHECKBOX, BORDER + (state * 3))),
                     BLANK);
...
int GuiComboBox(Rectangle bounds, const char* text, int* active)
{
    ...
    //--------------------------------------------------------------------
    // Draw combo box main
    GuiDrawRectangle(bounds, GuiGetStyle(COMBOBOX, BORDER_WIDTH), GetColor(GuiGetStyle(COMBOBOX, BORDER + (state * 3))),
                     GetColor(GuiGetStyle(COMBOBOX, BASE + (state * 3))));
    GuiDrawText(items[*active], GetTextBounds(COMBOBOX, bounds), GuiGetStyle(COMBOBOX, TEXT_ALIGNMENT),
...
int GuiDropdownBox(Rectangle bounds, const char* text, int* active, bool editMode)
{
    ...
    if (direction == 1) boundsOpen.y -= itemCount * (bounds.height + GuiGetStyle(DROPDOWNBOX, DROPDOWN_ITEMS_SPACING)) +
    ...
    GuiDrawText(items[itemSelected], GetTextBounds(DROPDOWNBOX, bounds), GuiGetStyle(DROPDOWNBOX, TEXT_ALIGNMENT),
    ...
    if (editMode)
    {
        ...
        for (int i = 0; i < itemCount; i++)
        {
            ...
            if (i == itemSelected)
            {
                ...
                GuiDrawText(items[i], GetTextBounds(DROPDOWNBOX, itemBounds), GuiGetStyle(DROPDOWNBOX, TEXT_ALIGNMENT),
            ...
            else GuiDrawText(items[i], GetTextBounds(DROPDOWNBOX, itemBounds), GuiGetStyle(DROPDOWNBOX, TEXT_ALIGNMENT),
...
#else
        GuiDrawText(direction ? "#121#" : "#120#", RAYGUI_CLITERAL(Rectangle){
...
#define RAYGUI_TEXTBOX_AUTO_CURSOR_DELAY      2        // Frames delay for autocursor movement
    ...
    if (cursor.y < (bounds.y + GuiGetStyle(TEXTBOX, BORDER_WIDTH))) cursor.y = bounds.y + GuiGetStyle(
        ...
        (wrapMode == TEXT_WRAP_NONE)) // No wrap mode
    {
        ...
        if (editMode)
        {
            ...
            if (IsKeyDown(KEY_LEFT) || IsKeyDown(KEY_RIGHT) || IsKeyDown(KEY_UP) || IsKeyDown(KEY_DOWN) ||
                IsKeyDown(KEY_BACKSPACE) || IsKeyDown(KEY_DELETE)) autoCursorCounter++;
            else autoCursorCounter = 0;

            bool autoCursorShouldTrigger = (autoCursorCounter > RAYGUI_TEXTBOX_AUTO_CURSOR_COOLDOWN) && ((
            ...
            if (IsKeyPressed(KEY_V) && (IsKeyDown(KEY_LEFT_CONTROL) || IsKeyDown(KEY_RIGHT_CONTROL)))
            {
                ...
                if (pasteText != NULL)
                {
                    ...
                    if (pasteLength > 0)
                    {
                        ...
                        for (int i = textLength + pasteLength; i > textBoxCursorIndex; i--) text[i] = text[i -
                            pasteLength];
            ...
            else if (((multiline && (codepoint == (int)'\n')) || (codepoint >= 32)) && ((textLength + codepointSize) <
                textSize))
            {
                // Move forward data from cursor position
                for (int i = (textLength + codepointSize); i > textBoxCursorIndex; i--) text[i] = text[i -
                    codepointSize];
            ...
            if ((textLength > textBoxCursorIndex) && IsKeyPressed(KEY_DELETE) && (IsKeyDown(KEY_LEFT_CONTROL) ||
                IsKeyDown(KEY_RIGHT_CONTROL)))
            {
                ...
                {
                    if ((puctuation && !ispunct(nextCodepoint & 0xFF)) || (!puctuation && (isspace(nextCodepoint & 0xFF)
                ...
                autoCursorShouldTrigger)))
            {
                ...
                // Move text after cursor forward (including final null terminator)
                for (int i = textBoxCursorIndex + nextCodepointSize; i <= textLength; i++) text[i - nextCodepointSize] =
                    text[i];
            ...
            if ((textBoxCursorIndex > 0) && IsKeyPressed(KEY_BACKSPACE) && (IsKeyDown(KEY_LEFT_CONTROL) || IsKeyDown(
                KEY_RIGHT_CONTROL)))
            {
                int offset = textBoxCursorIndex;
                int accCodepointSize = 0;
                ...
                {
                    prevCodepoint = GetCodepointPrevious(text + offset, &prevCodepointSize);
                    if ((puctuation && !ispunct(prevCodepoint & 0xFF)) || (!puctuation && (isspace(prevCodepoint & 0xFF)
                        || ispunct(prevCodepoint & 0xFF)))) break;
            ...
            if ((textBoxCursorIndex > 0) && IsKeyPressed(KEY_LEFT) && (IsKeyDown(KEY_LEFT_CONTROL) || IsKeyDown(
                KEY_RIGHT_CONTROL)))
            {
                int offset = textBoxCursorIndex;
                int accCodepointSize = 0;
            ...
            else if ((textBoxCursorIndex > 0) && (IsKeyPressed(KEY_LEFT) || (IsKeyDown(KEY_LEFT) &&
                autoCursorShouldTrigger)))
            {
                int prevCodepointSize = 0;
                GetCodepointPrevious(text + textBoxCursorIndex, &prevCodepointSize);
            ...
            else if ((textLength > textBoxCursorIndex) && IsKeyPressed(KEY_RIGHT) && (IsKeyDown(KEY_LEFT_CONTROL) ||
                IsKeyDown(KEY_RIGHT_CONTROL)))
            {
                int offset = textBoxCursorIndex;
                int accCodepointSize = 0;
                ...
                {
                    if ((puctuation && !ispunct(nextCodepoint & 0xFF)) || (!puctuation && (isspace(nextCodepoint & 0xFF)
                        || ispunct(nextCodepoint & 0xFF)))) break;
            ...
            else if ((textLength > textBoxCursorIndex) && (IsKeyPressed(KEY_RIGHT) || (IsKeyDown(KEY_RIGHT) &&
                autoCursorShouldTrigger)))
            {
                int nextCodepointSize = 0;
                GetCodepointNext(text + textBoxCursorIndex, &nextCodepointSize);
            ...
            if (CheckCollisionPointRec(mousePosition, textBounds)) // Mouse hover text
            {
                ...
                for (int i = textIndexOffset; i < textLength; i += codepointSize)
                {
                    ...
                    if (guiFont.glyphs[codepointIndex].advanceX == 0) glyphWidth = ((float)guiFont.recs[codepointIndex].
                        width * scaleFactor);
            ...
            cursor.x = bounds.x + GuiGetStyle(TEXTBOX, TEXT_PADDING) + GetTextWidth(text + textIndexOffset) -
                GetTextWidth(text + textBoxCursorIndex) + GuiGetStyle(DEFAULT, TEXT_SPACING);
    ...
    GuiDrawText(text + textIndexOffset, textBounds, GuiGetStyle(TEXTBOX, TEXT_ALIGNMENT),
    ...
    if (editMode && !GuiGetStyle(TEXTBOX, TEXT_READONLY))
    {
        ...
        if (mouseCursor.x >= 0) GuiDrawRectangle(mouseCursor, 0, BLANK,
...
int GuiSpinner(Rectangle bounds, const char* text, int* value, int minValue, int maxValue, bool editMode)
{
    ...
    Rectangle leftButtonBound = {
        (float)bounds.x, (float)bounds.y, (float)GuiGetStyle(VALUEBOX, SPINNER_BUTTON_WIDTH), (float)bounds.height
    };
    Rectangle rightButtonBound = {
        (float)bounds.x + bounds.width - GuiGetStyle(VALUEBOX, SPINNER_BUTTON_WIDTH), (float)bounds.y,
    ...
    if (text != NULL)
    {
        ...
        textBounds.y = bounds.y + bounds.height / 2 - GuiGetStyle(DEFAULT, TEXT_SIZE) / 2;
        if (GuiGetStyle(VALUEBOX, TEXT_ALIGNMENT) == TEXT_ALIGN_LEFT) textBounds.x = bounds.x - textBounds.width -
...
#define RAYGUI_VALUEBOX_MAX_CHARS  32
    ...
    if ((state != STATE_DISABLED) && !guiLocked && !guiControlExclusiveMode)
    {
        ...
        if (editMode)
        {
            ...
            if ((IsKeyPressed(KEY_ENTER) || IsKeyPressed(KEY_KP_ENTER)) || (!CheckCollisionPointRec(mousePoint, bounds)
...
int GuiValueBoxFloat(Rectangle bounds, const char* text, char* textValue, float* value, bool editMode)
{
    ...
    if ((state != STATE_DISABLED) && !guiLocked && !guiControlExclusiveMode)
    {
        ...
        if (editMode)
        {
            ...
            if ((IsKeyPressed(KEY_ENTER) || IsKeyPressed(KEY_KP_ENTER)) || (!CheckCollisionPointRec(mousePoint, bounds)
                && IsMouseButtonPressed(MOUSE_LEFT_BUTTON))) result = 1;
        }
        else
        {
            if (CheckCollisionPointRec(mousePoint, bounds))
...
int GuiSlider(Rectangle bounds, const char* textLeft, const char* textRight, float* value, float minValue,
              float maxValue)
{
    int result = 0;
    GuiState state = guiState;
    ...
    Rectangle slider = {
        bounds.x, bounds.y + GuiGetStyle(SLIDER, BORDER_WIDTH) + GuiGetStyle(SLIDER, SLIDER_PADDING),
        ...
        if (guiControlExclusiveMode) // Allows to keep dragging outside of bounds
        {
            if (IsMouseButtonDown(MOUSE_LEFT_BUTTON))
            {
                if (CHECK_BOUNDS_ID(bounds, guiControlExclusiveRec))
                {
                    ...
                    *value = (maxValue - minValue) * ((mousePoint.x - bounds.x - sliderWidth / 2) / (bounds.width -
                        sliderWidth)) + minValue;
                }
            }
            else
            {
    ...
    float sliderValue = (((*value - minValue) / (maxValue - minValue)) * (bounds.width - sliderWidth - 2 * GuiGetStyle(
        SLIDER, BORDER_WIDTH)));
    if (sliderWidth > 0) // Slider
    {
        slider.x += sliderValue;
        slider.width = (float)sliderWidth;
        if (slider.x <= (bounds.x + GuiGetStyle(SLIDER, BORDER_WIDTH))) slider.x = bounds.x + GuiGetStyle(
            SLIDER, BORDER_WIDTH);
        else if ((slider.x + slider.width) >= (bounds.x + bounds.width)) slider.x = bounds.x + bounds.width - slider.
            width - GuiGetStyle(SLIDER, BORDER_WIDTH);
    }
    ...
    // Draw control
    //--------------------------------------------------------------------
    GuiDrawRectangle(bounds, GuiGetStyle(SLIDER, BORDER_WIDTH), GetColor(GuiGetStyle(SLIDER, BORDER + (state * 3))),
    ...
    else if (state == STATE_DISABLED) GuiDrawRectangle(slider, 0, BLANK,
...
int GuiSliderBar(Rectangle bounds, const char* textLeft, const char* textRight, float* value, float minValue,
                 float maxValue)
{
    int result = 0;
...
int GuiProgressBar(Rectangle bounds, const char* textLeft, const char* textRight, float* value, float minValue,
                   float maxValue)
{
    int result = 0;
    GuiState state = guiState;
    ...
    if ((state != STATE_DISABLED)) progress.width = ((float)*value / (maxValue - minValue)) * (bounds.width - 2 *
    ...
    else
    {
        if (*value > minValue)
        {
            ...
            GuiDrawRectangle(
                RAYGUI_CLITERAL(Rectangle){
                    bounds.x, bounds.y + 1, (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH), bounds.height - 2
                }, 0, BLANK, GetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_FOCUSED)));
            GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){
        ...
        else GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){
        ...
        if (*value >= maxValue) GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){
                                                     bounds.x + progress.width + (float)GuiGetStyle(
        ...
        else
        {
            ...
            GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){
                                 bounds.x + (int)progress.width + (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH),
                                 bounds.y,
                                 bounds.width - (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH) - (int)progress.width - 1,
                                 (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH)
                             }, 0, BLANK, GetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_NORMAL)));
            GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){
                                 bounds.x + (int)progress.width + (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH),
            ...
            GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){
                                 bounds.x + bounds.width - (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH), bounds.y,
                                 (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH),
                                 bounds.height + GuiGetStyle(PROGRESSBAR, BORDER_WIDTH) - 1
                             }, 0, BLANK, GetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_NORMAL)));
        }
...
int GuiStatusBar(Rectangle bounds, const char* text)
{
    ...
    GuiDrawText(text, GetTextBounds(STATUSBAR, bounds), GuiGetStyle(STATUSBAR, TEXT_ALIGNMENT),
...
int GuiDummyRec(Rectangle bounds, const char* text)
{
    ...
    GuiDrawText(text, GetTextBounds(DEFAULT, bounds), TEXT_ALIGN_CENTER,
                GetColor(GuiGetStyle(BUTTON, (state != STATE_DISABLED) ? TEXT_COLOR_NORMAL : TEXT_COLOR_DISABLED)));
    //------------------------------------------------------------------
...
int GuiListViewEx(Rectangle bounds, const char** text, int count, int* scrollIndex, int* active, int* focus)
{
    ...
    int visibleItems = (int)bounds.height / (GuiGetStyle(LISTVIEW, LIST_ITEMS_HEIGHT) + GuiGetStyle(
        LISTVIEW, LIST_ITEMS_SPACING));
        ...
        else
        {
            if (((startIndex + i) == itemSelected) && (active != NULL))
            {
                ...
                GuiDrawText(text[startIndex + i], GetTextBounds(DEFAULT, itemBounds),
                            GuiGetStyle(LISTVIEW, TEXT_ALIGNMENT), GetColor(GuiGetStyle(LISTVIEW, TEXT_COLOR_PRESSED)));
            }
            ...
            // && (focus != NULL))  // NOTE: We want items focused, despite not returned!
            {
                ...
                GuiDrawText(text[startIndex + i], GetTextBounds(DEFAULT, itemBounds),
                            GuiGetStyle(LISTVIEW, TEXT_ALIGNMENT), GetColor(GuiGetStyle(LISTVIEW, TEXT_COLOR_FOCUSED)));
            }
            else
            {
                // Draw item normal (no rectangle)
                GuiDrawText(text[startIndex + i], GetTextBounds(DEFAULT, itemBounds),
                            GuiGetStyle(LISTVIEW, TEXT_ALIGNMENT), GetColor(GuiGetStyle(LISTVIEW, TEXT_COLOR_NORMAL)));
            }
        }
...
#define RAYGUI_COLORBARALPHA_CHECKED_SIZE   10
    ...
    GuiState state = guiState;
    Rectangle selector = {
        (float)bounds.x + (*alpha) * bounds.width - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_HEIGHT) / 2,
    ...
    if (state != STATE_DISABLED)
    {
        ...
        for (int x = 0; x < checksX; x++)
        {
            ...
            {
                Rectangle check = {
                    bounds.x + x * RAYGUI_COLORBARALPHA_CHECKED_SIZE, bounds.y + y * RAYGUI_COLORBARALPHA_CHECKED_SIZE,
        ...
        DrawRectangleGradientEx(bounds, RAYGUI_CLITERAL(Color){255, 255, 255, 0},
                                RAYGUI_CLITERAL(Color){255, 255, 255, 0},
                                Fade(RAYGUI_CLITERAL(Color){0, 0, 0, 255}, guiAlpha),
                                Fade(RAYGUI_CLITERAL(Color){0, 0, 0, 255}, guiAlpha));
    }
    else DrawRectangleGradientEx(bounds, Fade(GetColor(GuiGetStyle(COLORPICKER, BASE_COLOR_DISABLED)), 0.1f),
...
int GuiColorBarHue(Rectangle bounds, const char* text, float* hue)
{
    ...
    if (state != STATE_DISABLED)
    {
        ...
        DrawRectangleGradientV((int)bounds.x, (int)(bounds.y), (int)bounds.width, (int)ceilf(bounds.height / 6),
                               Fade(RAYGUI_CLITERAL(Color){255, 0, 0, 255}, guiAlpha),
                               Fade(RAYGUI_CLITERAL(Color){255, 255, 0, 255}, guiAlpha));
        DrawRectangleGradientV((int)bounds.x, (int)(bounds.y + bounds.height / 6), (int)bounds.width,
                               (int)ceilf(bounds.height / 6), Fade(RAYGUI_CLITERAL(Color){255, 255, 0, 255}, guiAlpha),
                               Fade(RAYGUI_CLITERAL(Color){0, 255, 0, 255}, guiAlpha));
        DrawRectangleGradientV((int)bounds.x, (int)(bounds.y + 2 * (bounds.height / 6)), (int)bounds.width,
                               (int)ceilf(bounds.height / 6), Fade(RAYGUI_CLITERAL(Color){0, 255, 0, 255}, guiAlpha),
                               Fade(RAYGUI_CLITERAL(Color){0, 255, 255, 255}, guiAlpha));
        DrawRectangleGradientV((int)bounds.x, (int)(bounds.y + 3 * (bounds.height / 6)), (int)bounds.width,
                               (int)ceilf(bounds.height / 6), Fade(RAYGUI_CLITERAL(Color){0, 255, 255, 255}, guiAlpha),
                               Fade(RAYGUI_CLITERAL(Color){0, 0, 255, 255}, guiAlpha));
        DrawRectangleGradientV((int)bounds.x, (int)(bounds.y + 4 * (bounds.height / 6)), (int)bounds.width,
                               (int)ceilf(bounds.height / 6), Fade(RAYGUI_CLITERAL(Color){0, 0, 255, 255}, guiAlpha),
                               Fade(RAYGUI_CLITERAL(Color){255, 0, 255, 255}, guiAlpha));
        DrawRectangleGradientV((int)bounds.x, (int)(bounds.y + 5 * (bounds.height / 6)), (int)bounds.width,
                               (int)(bounds.height / 6), Fade(RAYGUI_CLITERAL(Color){255, 0, 255, 255}, guiAlpha),
                               Fade(RAYGUI_CLITERAL(Color){255, 0, 0, 255}, guiAlpha));
    }
    else DrawRectangleGradientV((int)bounds.x, (int)bounds.y, (int)bounds.width, (int)bounds.height,
...
int GuiColorPicker(Rectangle bounds, const char* text, Color* color)
{
    ...
    Rectangle boundsHue = {
        (float)bounds.x + bounds.width + GuiGetStyle(COLORPICKER, HUEBAR_PADDING), (float)bounds.y,
...
int GuiColorPanelHSV(Rectangle bounds, const char* text, Vector3* colorHsv)
{
    ...
    if (state != STATE_DISABLED)
    {
        DrawRectangleGradientEx(bounds, Fade(colWhite, guiAlpha), Fade(colWhite, guiAlpha), Fade(maxHueCol, guiAlpha),
                                Fade(maxHueCol, guiAlpha));
        DrawRectangleGradientEx(bounds, Fade(colBlack, 0), Fade(colBlack, guiAlpha), Fade(colBlack, guiAlpha),
                                Fade(colBlack, 0));
        ...
        Rectangle selector = {
            ...
            (float)GuiGetStyle(COLORPICKER, COLOR_SELECTOR_SIZE), (float)GuiGetStyle(COLORPICKER, COLOR_SELECTOR_SIZE)
        };
    ...
    else
    {
        DrawRectangleGradientEx(
            bounds, Fade(Fade(GetColor(GuiGetStyle(COLORPICKER, BASE_COLOR_DISABLED)), 0.1f), guiAlpha),
...
#define RAYGUI_MESSAGEBOX_BUTTON_PADDING   12
    ...
    textBounds.height = bounds.height - RAYGUI_WINDOWBOX_STATUSBAR_HEIGHT - 3 * RAYGUI_MESSAGEBOX_BUTTON_PADDING -
...
#define RAYGUI_TEXTINPUTBOX_HEIGHT             26
    ...
    int messageInputHeight = (int)bounds.height - RAYGUI_WINDOWBOX_STATUSBAR_HEIGHT -
        GuiGetStyle(STATUSBAR, BORDER_WIDTH) - RAYGUI_TEXTINPUTBOX_BUTTON_HEIGHT - 2 *
    ...
    if (message != NULL)
    {
        ...
        textBounds.y = bounds.y + RAYGUI_WINDOWBOX_STATUSBAR_HEIGHT + messageInputHeight / 4 - (float)GuiGetStyle(
            DEFAULT, TEXT_SIZE) / 2;
    ...
    {
        static char stars[] = "****************";
        if (GuiTextBox(RAYGUI_CLITERAL(Rectangle){
                           textBoxBounds.x, textBoxBounds.y, textBoxBounds.width - 4 - RAYGUI_TEXTINPUTBOX_HEIGHT,
                           textBoxBounds.height
                       },
                       ((*secretViewActive == 1) || textEditMode) ? text : stars, textMaxSize, textEditMode))
            textEditMode = !textEditMode;

        GuiToggle(RAYGUI_CLITERAL(Rectangle){
                ...
                case 'f':
                    {
                        ...
                        if (fontFileName[0] != '\0')
                        {
                            ...
                            if (codepointCount > 0) font = LoadFontEx(
                                TextFormat("%s/%s", GetDirectoryPath(fileName), fontFileName), fontSize, codepoints,
                                codepointCount);
                            else font = LoadFontEx(TextFormat("%s/%s", GetDirectoryPath(fileName), fontFileName),
...
void GuiLoadStyleDefault(void)
{
    ...
    if (guiFont.texture.id != GetFontDefault().texture.id)
    {
        ...
        SetShapesTexture(guiFont.texture, RAYGUI_CLITERAL(Rectangle){
...
#if !defined(RAYGUI_STANDALONE)
                GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){
                                     (float)posX + (k % RAYGUI_ICON_SIZE) * pixelSize, (float)posY + y * pixelSize,
        ...
        (signature[3] == ' '))
    {
        ...
        for (int i = 0; i < propertyCount; i++)
        {
            ...
            if (controlId == 0) // DEFAULT control
            {
                ...
                if (propertyId < RAYGUI_MAX_PROPS_BASE) for (int j = 1; j < RAYGUI_MAX_CONTROLS; j++) GuiSetStyle(
                    j, (int)propertyId, propertyValue);
            }
        ...
        if (fontDataSize > 0)
        {
            ...
            if (font.texture.id != 0)
            {
                ...
                if ((recsDataCompressedSize > 0) && (recsDataCompressedSize != recsDataSize))
                {
                    ...
                    font.recs = (Rectangle*)DecompressData(recsDataCompressed, recsDataCompressedSize,
                                                           &recsDataUncompSize);
                ...
                if ((glyphsDataCompressedSize > 0) && (glyphsDataCompressedSize != glyphsDataSize))
                {
                    ...
                    unsigned char* glyphsDataUncomp = DecompressData(glypsDataCompressed, glyphsDataCompressedSize,
    ...
    default:
        {
            ...
            if (GuiGetStyle(control, TEXT_ALIGNMENT) == TEXT_ALIGN_RIGHT) textBounds.x -= GuiGetStyle(
                control, TEXT_PADDING);
...
#define TEXT_VALIGN_PIXEL_OFFSET(h)  ((int)h%2)     // Vertical alignment for pixel perfect
    ...
    float totalHeight = (float)(lineCount * GuiGetStyle(DEFAULT, TEXT_SIZE) + (lineCount - 1) * GuiGetStyle(
        DEFAULT, TEXT_SIZE) / 2);
    ...
    for (int i = 0; i < lineCount; i++)
    {
        ...
        switch (alignment)
        {
        case TEXT_ALIGN_LEFT: textBoundsPosition.x = textBounds.x;
            break;
        case TEXT_ALIGN_CENTER: textBoundsPosition.x = textBounds.x + textBounds.width / 2 - textSizeX / 2;
            break;
        case TEXT_ALIGN_RIGHT: textBoundsPosition.x = textBounds.x + textBounds.width - textSizeX;
            break;
        default: break;
        }

        if (textSizeX > textBounds.width && (lines[i] != NULL) && (lines[i][0] != '\0')) textBoundsPosition.x =
            textBounds.x;

        switch (alignmentVertical)
        {
        // Only valid in case of wordWrap = 0;
        case TEXT_ALIGN_TOP: textBoundsPosition.y = textBounds.y + posOffsetY;
            break;
        case TEXT_ALIGN_MIDDLE: textBoundsPosition.y = textBounds.y + posOffsetY + textBounds.height / 2 - totalHeight /
                2 + TEXT_VALIGN_PIXEL_OFFSET(textBounds.height);
            break;
        case TEXT_ALIGN_BOTTOM: textBoundsPosition.y = textBounds.y + posOffsetY + textBounds.height - totalHeight +
        ...
        if (iconId >= 0)
        {
            ...
            GuiDrawIcon(iconId, (int)textBoundsPosition.x,
                        (int)(textBounds.y + textBounds.height / 2 - RAYGUI_ICON_SIZE * guiIconScale / 2 +
                            TEXT_VALIGN_PIXEL_OFFSET(textBounds.height)), guiIconScale, tint);
        ...
        for (int c = 0, codepointSize = 0; c < lineSize; c += codepointSize)
        {
            ...
            else
            {
                ...
                if ((codepoint != ' ') && (codepoint != '\t')) // Do not draw codepoints with no glyph
                {
                    if (wrapMode == TEXT_WRAP_NONE)
                    {
                        ...
                        else
                        {
                            DrawTextCodepoint(guiFont, codepoint,
                                              RAYGUI_CLITERAL(Vector2){
                                                  textBoundsPosition.x + textOffsetX, textBoundsPosition.y + textOffsetY
                                              }, (float)GuiGetStyle(DEFAULT, TEXT_SIZE), GuiFade(tint, guiAlpha));
                        }
                    }
                    else if ((wrapMode == TEXT_WRAP_CHAR) || (wrapMode == TEXT_WRAP_WORD))
                    {
                        ...
                        if ((textBoundsPosition.y + textOffsetY) <= (textBounds.y + textBounds.height - GuiGetStyle(
                            DEFAULT, TEXT_SIZE)))
                        {
                            DrawTextCodepoint(guiFont, codepoint,
                                              RAYGUI_CLITERAL(Vector2){
                ...
                if (guiFont.glyphs[index].advanceX == 0) textOffsetX += ((float)guiFont.recs[index].width * scaleFactor
                    + (float)GuiGetStyle(DEFAULT, TEXT_SPACING));
                else textOffsetX += ((float)guiFont.glyphs[index].advanceX * scaleFactor + (float)GuiGetStyle(
                    DEFAULT, TEXT_SPACING));
            }
        }

        if (wrapMode == TEXT_WRAP_NONE) posOffsetY += (float)GuiGetStyle(DEFAULT, TEXT_LINE_SPACING);
        else if ((wrapMode == TEXT_WRAP_CHAR) || (wrapMode == TEXT_WRAP_WORD)) posOffsetY += (textOffsetY + (float)
...
static void GuiTooltip(Rectangle controlRec)
{
    ...
    {
        Vector2 textSize = MeasureTextEx(GuiGetFont(), guiTooltipPtr, (float)GuiGetStyle(DEFAULT, TEXT_SIZE),
                                         (float)GuiGetStyle(DEFAULT, TEXT_SPACING));

        if ((controlRec.x + textSize.x + 16) > GetScreenWidth()) controlRec.x -= (textSize.x + 16 - controlRec.width);

        GuiPanel(RAYGUI_CLITERAL(Rectangle){
                     controlRec.x, controlRec.y + controlRec.height + 4, textSize.x + 16,
                     GuiGetStyle(DEFAULT, TEXT_SIZE) + 8.0f
                 }, NULL);
        ...
        GuiSetStyle(LABEL, TEXT_ALIGNMENT, TEXT_ALIGN_CENTER);
        GuiLabel(RAYGUI_CLITERAL(Rectangle){
                     controlRec.x, controlRec.y + controlRec.height + 4, textSize.x + 16,
                     GuiGetStyle(DEFAULT, TEXT_SIZE) + 8.0f
                 }, guiTooltipPtr);
...
static int GuiScrollBar(Rectangle bounds, int value, int minValue, int maxValue)
{
    ...
    if (isVertical)
    {
        arrowDownRight = RAYGUI_CLITERAL(Rectangle){
            (float)bounds.x + GuiGetStyle(SCROLLBAR, BORDER_WIDTH),
            (float)bounds.y + bounds.height - spinnerSize - GuiGetStyle(SCROLLBAR, BORDER_WIDTH), (float)spinnerSize,
            (float)spinnerSize
        };
        scrollbar = RAYGUI_CLITERAL(Rectangle){
            ...
            bounds.width - 2 * (GuiGetStyle(SCROLLBAR, BORDER_WIDTH) + GuiGetStyle(SCROLLBAR, SCROLL_PADDING)),
            bounds.height - arrowUpLeft.height - arrowDownRight.height - 2 * GuiGetStyle(SCROLLBAR, BORDER_WIDTH)
        };
    ...
    else // horizontal
    {
        arrowDownRight = RAYGUI_CLITERAL(Rectangle){
            ...
            (float)bounds.y + GuiGetStyle(SCROLLBAR, BORDER_WIDTH), (float)spinnerSize, (float)spinnerSize
        };
        scrollbar = RAYGUI_CLITERAL(Rectangle){
            arrowUpLeft.x + arrowUpLeft.width,
            bounds.y + GuiGetStyle(SCROLLBAR, BORDER_WIDTH) + GuiGetStyle(SCROLLBAR, SCROLL_PADDING),
            bounds.width - arrowUpLeft.width - arrowDownRight.width - 2 * GuiGetStyle(SCROLLBAR, BORDER_WIDTH),
            bounds.height - 2 * (GuiGetStyle(SCROLLBAR, BORDER_WIDTH) + GuiGetStyle(SCROLLBAR, SCROLL_PADDING))
        };
    ...
    if ((state != STATE_DISABLED) && !guiLocked)
    {
        ...
        if (guiControlExclusiveMode) // Allows to keep dragging outside of bounds
        {
                ...
                !CheckCollisionPointRec(mousePoint, arrowDownRight))
            {
                if (CHECK_BOUNDS_ID(bounds, guiControlExclusiveRec))
                {
                    ...
                    if (isVertical) value = (int)(((float)(mousePoint.y - scrollbar.y - slider.height / 2) * valueRange)
                        / (scrollbar.height - slider.height) + minValue);
                    else value = (int)(((float)(mousePoint.x - scrollbar.x - slider.width / 2) * valueRange) / (
                        scrollbar.width - slider.width) + minValue);
                }
            }
            else
            {
        ...
        else if (CheckCollisionPointRec(mousePoint, bounds))
        {
            ...
            if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON))
            {
                ...
                // Check arrows click
                if (CheckCollisionPointRec(mousePoint, arrowUpLeft)) value -= valueRange / GuiGetStyle(
                    SCROLLBAR, SCROLL_SPEED);
                else if (CheckCollisionPointRec(mousePoint, arrowDownRight)) value += valueRange / GuiGetStyle(
                    SCROLLBAR, SCROLL_SPEED);
    ...
    // Draw control
    //--------------------------------------------------------------------
    GuiDrawRectangle(bounds, GuiGetStyle(SCROLLBAR, BORDER_WIDTH), GetColor(GuiGetStyle(LISTVIEW, BORDER + state * 3)),
                     GetColor(GuiGetStyle(DEFAULT, BORDER_COLOR_DISABLED))); // Draw the background

    GuiDrawRectangle(scrollbar, 0, BLANK, GetColor(GuiGetStyle(BUTTON, BASE_COLOR_NORMAL)));
    ...
    if (GuiGetStyle(SCROLLBAR, ARROWS_VISIBLE))
    {
        ...
        GuiDrawText(isVertical ? "#121#" : "#118#",
                    ...
                    TEXT_ALIGN_CENTER,
                    GetColor(GuiGetStyle(SCROLLBAR, TEXT + state * 3))); // ICON_ARROW_UP_FILL / ICON_ARROW_LEFT_FILL
        GuiDrawText(isVertical ? "#120#" : "#119#",
                    ...
                    TEXT_ALIGN_CENTER,
                    GetColor(GuiGetStyle(SCROLLBAR, TEXT + state * 3))); // ICON_ARROW_DOWN_FILL / ICON_ARROW_RIGHT_FILL
#endif
    }
    //--------------------------------------------------------------------
